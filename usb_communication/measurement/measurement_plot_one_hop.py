...
#===========================================
    ...
    ax.set_title ('single hop (channel loss ~%d%%)' % getAverageChannelLoss (channel_loss_estimation), fontsize=20)
...
#===========================================
    ...
    for index in range (len (data_no_src)):
        if data_no_src[index]['type'] == 'no_coding':
            no_coding_loss_rate.append (data_no_dst[index]['loss_rate'])
            no_coding_tx_num.append (data_no_src[index]['tx_num'])
    ...
    if plot_loss == True:
        ...
        ax.set_title ('single hop (channel loss ~%d%%)' % getAverageChannelLoss (average_channel_loss_estimation), fontsize=20)
        ax.set_xticks (x)
